...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const parsedStages = JSON.parse(saved);
        // Merge saved completion status with static data structure
        // We MUST map over the static STAGES to preserve function/component references (like icons)
        // which are lost during JSON serialization
        setStages(currentStages => {
            return currentStages.map(stage => {
                const savedStage = parsedStages.find((s: any) => s.id === stage.id);
                if (!savedStage) return stage;
                
                return {
                    ...stage,
                    tasks: stage.tasks.map(task => {
                        const savedTask = savedStage.tasks?.find((t: any) => t.id === task.id);
